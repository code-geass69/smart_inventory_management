...
type AwaitedBrand = Pick<Brand, "id" | "name" | "category">
...
export function BrandsTableShell({
  data,
  pageCount,
}: BrandsTableShellProps): JSX.Element {
  const { toast } = useToast()
  const router = useRouter()
  const [isPending, startTransition] = React.useTransition()
  const [selectedRowIds, setSelectedRowIds] = React.useState<number[]>([])

  const columns = React.useMemo<ColumnDef<AwaitedBrand, unknown>[]>(
    () => [
      {
        id: "select",
        header: ({ table }) => (
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-[160px]">
              <DropdownMenuItem asChild className="cursor-pointer">
                <Link
                  href={`/app/inventory/brands/${row.original.id}`}
                  className="text-sm"
                >
                  Edit
                </Link>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      ...
                      delete your brand from the database.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    ...
                    <AlertDialogAction
                      onClick={() => {
                        startTransition(async () => {
                          try {
                            const message = await deleteBrand({
                              id: row.original.id,
                            })
